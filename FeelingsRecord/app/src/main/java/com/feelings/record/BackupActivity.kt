...
import com.feelings.record.util.DriveServiceHelper
import com.feelings.record.util.RunTimePermission
...
class BackupActivity : RunTimePermission() {
                ...
                if (isGranted) {
                    ...
                    if (mDriveServiceHelper == null)
                        ...
                        uploadTask?.addOnCompleteListener {
                            ...
                            kotlin.io.println("lastUploadFileId==>$lastUploadFileId")
                            Toast.makeText(this@BackupActivity, "Backup upload successfully", Toast.LENGTH_LONG).show()
                        }
                    }
                }
            }
                    ...
                    if (mDriveServiceHelper == null)
                        ...
                        if (null != lastUploadFileId) {
                            ...
                            downloadTask?.addOnCompleteListener {
                                kotlin.io.println("Name==>${downloadTask.result?.first}")
                                kotlin.io.println("Content==>${downloadTask.result?.second}")
                                Toast.makeText(this@BackupActivity, "Backup download successfully", Toast.LENGTH_LONG)
                                        .show()
                            }
                        }
                    }
                ...
                if (isRestore) {
                    if (null != lastUploadFileId) {
                        ...
                        downloadTask?.addOnCompleteListener {
                            ...
                            Toast.makeText(this@BackupActivity, "Backup download successfully", Toast.LENGTH_LONG).show()
                        }
                    }
                } else {
                    val uploadTask = mDriveServiceHelper?.uploadFile(BACKUP_FILE, generateFile())
